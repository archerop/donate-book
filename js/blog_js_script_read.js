...
*/
	var db_act;
function startScript(canvasId,	play_record_json)
{ 
		...
		function playRecordings()
		{
			...
			if (btnTxt == 'Stop')
				stopPlayback();
					...
					startPlayback();		//startPlayback();  //good
					
		}
			function playRecordings_end()
		{
			if (drawing.recordings.length == 0)
			{
				alert("No recording to play");
			...
			if (btnTxt == 'Stop')
				stopPlayback();
					...
					startPlayback_end();		//startPlayback();  //good
...
$("#read").click(function(){
	...
	var result = deserializeDrawing_l(play_record_json);
	...
	function startPlayback_end()
	{
	//ok
		drawing.playRecording_end(function() {
...
RecordableDrawing = function (canvasId) //建立物件
{
	var self = this;
	this.canvas = null;
...
this.playRecording_end = function(callbackFunctionArg, onPlayEnd, onPause, interruptActionStatus)
	{
		...
		self.scheduleDraw_end(self.actionsSet,self.actionsSet.interval,callbackFunctionArg, onPlayEnd, onPause, true, interruptActionStatus);
	...
	//畫最後一張圖
		this.scheduleDraw_end = function (actionSetArg, interval, callbackFunctionArg, onPlayEnd, onPause, isFirst, interruptActionStatus)
	{
		window.setTimeout(function(){
			var status = "";
			if (interruptActionStatus != null)
			...
			if (intervalDiff >= 0)
				self.scheduleDraw_end(actionSetArg.next, intervalDiff, callbackFunctionArg, onPlayEnd, onPause, false,interruptActionStatus);
