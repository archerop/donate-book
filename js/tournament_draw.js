	...
	document.getElementById("recorder_submit").onclick = function() {
		...
		var serResult = serializeDrawing_l(drawing);
			...
			{	
				...
				//alert(answer2);return ;
				$.ajax({
					url: "insert_tournament.php",
					type: "POST",
					//data: 'number1='+number1+'&answer=' + answer+'&tournament_id=' + tournament_id,
					...
					//	console.log(json);
						var tournament_ref=$('#tournament_ref').val();
						if(json['result']=="0"){
							alert("你答錯了!!");
						}
						if(json['result']=="1"){
						...
						if(json['success']=="0"){
							alert("闖關失敗!!\n請重新再來!!");
							self.location.href='tournament_student_list.php'+tournament_ref;
							return;
						}
						if(json['success']=="1"){
							alert("恭喜你闖關成功!!");
						...
						if(json['success']=="2"){
							self.location.href='tournament_finish.php';
							return;
						}
						if(top.tournament_list==""){
							top.tournament_list=number1;
						}else{
							top.tournament_list=top.tournament_list+','+number1;
						}
						self.location.href='question_tournament.php?tournament_id='+tournament_id+'&tournament_list='+top.tournament_list;
		
						//$("#select_unit").empty().append('<option value="">不分</option>').append(message);
					} 
				});
			}else
			{
